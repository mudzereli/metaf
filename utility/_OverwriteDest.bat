...
set metOutDir=C:\Turbine\Asheron's Call\plugins\VirindiPlugins\VirindiTank\utility
set afOutDir=C:\Turbine\Asheron's Call\metaf\af
set metafDir=C:\Turbine\Asheron's Call\metaf\
REM ================================================================================
...
for /L %%i in (1,1,%argc%) do (
   ...
   if /i "!exts[%%i]!"==".af" (
	  "%metafDir%metaf.exe" "!argv[%%i]!" "%metOutDir%"
   ) else (
	  ""%metafDir%metaf.exe" "!argv[%%i]!" "%afOutDir%"
   )
)
